...
Локальное и серверное fullstack-приложение для учета участников, абонементов, занятий, практик аренды зала, внештатных расходов и заработка преподавателей.
...
- «Практика» — оплаченная аренда зала внешним человеком или зарегистрированным преподавателем; вся сумма является доходом школы.
...
- «Внештатные расходы» — нерегулярные траты по дате расхода; они не смешиваются с обязательными платежами, но входят в общий `expenses_total`.
...
Раздел `/practice` хранит практики отдельно от расписания и посещений. Запись практики не создает `ScheduleEvent`, не создает `Visit`, не списывает абонемент и не начисляет выплату преподавателю. Цена и название тарифа сохраняются snapshot-полями в момент создания записи.

Раздел `/extra-expenses` хранит нерегулярные расходы школы по модели «название + сумма + дата». Отмененные расходы остаются в истории, но исключаются из финансовых итогов.
...
- `/participants/:id` — карточка участника с историей абонементов и посещений.
- `/memberships` — таблица абонементов.
- `/finance` — продажи, стоимость проведенных занятий, выплаты преподавателям и доход школы.
